...
struct _syncmsg1
{
	...
	int key_action; // 这一次按键行为：上一张、下一张、删除
};
...
int cgiMain()
{
...
#if 1
	printf("cgi show_photo \n");
	printf("number = %s, index = %s, action = %s\n", syncmsg2.number, syncmsg2.index, syncmsg2.action);
#endif
	...
	else
	{
...
#if 0
		printf("syncmsg2.action = %s\n", syncmsg2.action);
		printf("syncmsg2.action[2] = %#x\n", syncmsg2.action[2]);
#endif
		...
		else if(syncmsg2.action[2] == 0xa0){
			syncmsg1.key_action = 3;       //删除照片
			//		printf("syncmsg1.key_action = %d\n", syncmsg1.key_action);
		}
		else if(syncmsg2.action[2] == 0xa8){
			syncmsg1.key_action = 4;       //删除全部照片
		}
	}
	...
	fprintf(cgiOut, 		"<td height=\"30\" width=\"10%%\"><input style=\"display:none\" name=\"number\" type=\"text\" id=\"delete\" value=\"%s\" size=\"10\" maxlength=\"100\"/></td>", syncmsg2.number);
	fprintf(cgiOut, 		"<td height=\"30\" width=\"15%%\" align=\"center\"><input type=\"submit\" name=\"action\" id=\"dele\" value=\"删除\" /></td>");
	fprintf(cgiOut, 		"<td height=\"30\" width=\"10%%\"><input style=\"display:none\" name=\"index\" type=\"text\" id=\"deleteall\" value=\"%s\" size=\"10\" maxlength=\"100\"/></td>", syncmsg2.index);
	fprintf(cgiOut, 		"<td height=\"30\" width=\"15%%\" align=\"center\"><input type=\"submit\" name=\"action\" id=\"deleall\" value=\"全部删除\" /></td>"); 
